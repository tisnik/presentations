...
// s vyuzitim algoritmu nahodne prochazky (RWA - Random Walk Algorithm).
...
//-----------------------------------------------------------------------------
// Prekresleni systemu iterovanych funkci IFS pomoci algoritmu RWA
//-----------------------------------------------------------------------------
void recalcIFSusingRWA(pixmap *pix,             // pixmapa pro vykreslovani
...
void onDisplay(void)
{
    ...
    if (redraw) {
        clearPixmap(pixIFS);
        recalcIFSusingRWA(pixIFS, maxiter, scale, xpos, ypos, ifs, rf, gf, bf, deltar, deltag, deltab, drawType);// prepocet fraktalu
        redraw=0;
    }
