...
// Ukazkovy priklad cislo 21.2
// Autor: Pavel Tisnovsky
//
// Vykresleni Juliovy mnoziny s libovolnou (i necelociselnou) mocninou.
...
#define WINDOW_TITLE    "Fraktaly 21.2"         // titulek okna
#define WINDOW_WIDTH    640                     // pocatecni velikost okna
#define WINDOW_HEIGHT   460
#define FILE_NAME       "fraktaly212.tga"       // jmeno souboru pro ulozeni pixmapy
...
//-----------------------------------------------------------------------------
// Prekresleni Juliovy mnoziny s libovolnou mocninou hodnoty Z(n)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void drawInfo(int maxiter, double xpos, double ypos, double scale, int palette, int r, int g, int b, double power, double cx, double cy)
{
    char str[100];
    char *palNames[10]={
        "blues",
...
void onDisplay(void)
{
    ...
    if (ma) {                                   // pokud se ma prepocitat Mandelbrotova mnozina
        recalcMandelbrot(pixMandel, maxiter, scale, xpos, ypos, power, palette, rf, gf, bf);// prepocet fraktalu
        ma=0;
    }
    if (mb) {                                   // pokud se ma prepocitat Juliova mnozina
        recalcJulia(pixJulia, maxiter, scale, xpos, ypos, power, palette, rf, gf, bf, pcx, pcy);// prepocet fraktalu
        mb=0;
    }
    ...
    drawPixmap(pixJulia);
    glRasterPos2i(0, 0);
    drawInfo(maxiter, xpos, ypos, scale, palette, rf, gf, bf, power, pcx, pcy);
...
void onKeyboard(unsigned char key, int x, int y)
{
    ...
    switch (key) {
        ...
        case 'z': power-=0.1;       ma=mb=1;     glutPostRedisplay();  break;
        case 'x': power+=0.1;       ma=mb=1;     glutPostRedisplay();  break;
        default:                                                       break; 
    }
}
#ifdef __BORLANDC__
