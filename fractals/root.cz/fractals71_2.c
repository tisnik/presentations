...
//
// Demonstracni priklad na vypocet moare s ruznymi vzorky
//-----------------------------------------------------------------------------
...
// pixmapa by mela byt ctvercova
#define PIXMAP_WIDTH    400                     // sirka pixmapy
#define PIXMAP_HEIGHT   400                     // vyska pixmapy
...
//-----------------------------------------------------------------------------
double xyfun2(double x, double y)
{
    return x*x*x+y*y*x;
}
...
//-----------------------------------------------------------------------------
// Funkce provadejici vypocet moare s obecnym vzorkem
//-----------------------------------------------------------------------------
void recalcPattern(pixmap *pix,                 // pracovni pixmapa
                    int    palette,             // cislo barvove palety
                    double xmin,                // meze fraktalu v rovine
                    double ymin,
                    double xmax,
                    double ymax,
                    double (*fce)(double, double)) // funkce provadejici vlastni vypocet
{
    int x, y, i;                                // pocitadla smycek
    double x1, y1;                              // prepoctena pozice pixelu
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky pixmapy
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            ...
            i=(double)fce(x1, y1);              // vypocet vzorku
...
int main(int argc, char **argv)
{
    ...
    // vykreslit nekolik obrazku s moare
    for (i=0; i<50; i++) {
        char str[20];
        double mez;
        sprintf(str, "moare%02d.tga", i);
        printf("making image '%s'\n", str);
        // zde je mozne zmenit meze a funkci pro vypocet moare
        mez=(2<<3)+i*2.5;
        recalcCirclePattern(pix, i%10, 0, 0, mez+15, mez+15, xyfun2);
        savePixmap(pix, str);
    }
    puts("all done");
    return 0;
