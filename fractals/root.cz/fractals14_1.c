...
// Ukazkovy priklad cislo 14.1
// Autor: Pavel Tisnovsky
//
// Vykresleni Mandelbrotovy mnoziny s moznosti zmeny pohledu na mnozinu.
...
#define WINDOW_TITLE    "Fraktaly 14.1"         // titulek okna
#define WINDOW_WIDTH    512                     // pocatecni velikost okna
#define WINDOW_HEIGHT   384
#define FILE_NAME_1     "fraktaly141m.tga"      // jmeno souboru pro ulozeni pixmapy Mandelbrotovy mnoziny
#define FILE_NAME_2     "fraktaly141j.tga"      // jmeno souboru pro ulozeni pixmapy Juliovy mnoziny
...
pixmap *pixMandel;                              // pixmapa pro obrazek Mandelbrotovy mnoziny
pixmap *pixJulia;                               // pixmapa pro obrazek Juliovy mnoziny

int    mf=1, jf=1;                              // priznaky pro prekresleni fraktalu
int    maxiter=128;                             // maximalni pocet iteraci
int    mouse=0;                                 // stav tlacitek mysi

double cx=0.0;                                  // hodnota komplexni konstanty C
double cy=0.0;
...
//-----------------------------------------------------------------------------
// Vypocet a prekresleni Juliovy mnoziny
//-----------------------------------------------------------------------------
void recalcJulia(pixmap  *pix,                  // pixmapa pro vykreslovani
                  int    maxiter,               // maximalni pocet iteraci
                  double cx,                    // hodnota komplexni konstanty C
                  double cy)
{
    double zx, zy, zx2, zy2;                    // slozky komplexni promenne Z a Z^2
    double zx0, zy0;
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            ...
            if (iter==maxiter)
                ...
                putpixel(pix, x, y, r, g, b);
            zx0+=(4.0)/pix->width;              // posun na dalsi bod na radku
        }
        zy0+=(4.0)/pix->height;                 // posun na dalsi radek
    }
}
...
void onInit(void)
{
    ...
    pixMandel=createPixmap(PIXMAP_WIDTH, PIXMAP_HEIGHT);
    pixJulia=createPixmap(PIXMAP_WIDTH, PIXMAP_HEIGHT);
}
...
void onDisplay(void)
{
    ...
    if (mf) {                                   // pokud se ma prepocitat Mandelbrotova mnozina
        recalcMandelbrot(pixMandel, maxiter);   // prepocet Mandelbrotovy mnoziny
        mf=0;
    }
    glRasterPos2i(0, WINDOW_HEIGHT-PIXMAP_HEIGHT); // nastaveni souradnic leveho spodniho rohu pixmapy
    drawPixmap(pixMandel);                      // vykresleni pixmapy

    if (jf) {                                   // pokud se ma prepocitat Juliova mnozina
        recalcJulia(pixJulia, maxiter, cx, cy);
        jf=0;
    }
    ...
    glRasterPos2i(0, 0);
    drawInfo(maxiter, cx, cy);                  // vypsani informaci o fraktalu
...
void onKeyboard(unsigned char key, int x, int y)
{
    ...
    switch (key) {
        ...
        case 's':
                  savePixmap(pixMandel, FILE_NAME_1);                         // ulozeni obou pixmap
                  savePixmap(pixJulia, FILE_NAME_2); 
                  break;
        case '<': if (maxiter>10)  mf=1; maxiter-=10; glutPostRedisplay(); break; 
        case ',': if (maxiter>1)   mf=1; maxiter--;   glutPostRedisplay(); break; // zmena poctu iteraci
        case '>': if (maxiter<246) mf=1; maxiter+=10; glutPostRedisplay(); break; 
        case '.': if (maxiter<255) mf=1; maxiter++;   glutPostRedisplay(); break; 
        default:                                                           break; 
    }
}
#ifdef __BORLANDC__
...
void onMouseMotion(int x, int y)
{
    if (mouse) {
        if (x>=0 && x<=PIXMAP_WIDTH && y>=0 && y<=PIXMAP_HEIGHT) {
            cx=4.0*x/PIXMAP_WIDTH-2.0;          // vypocet hodnoty
            cy=2.0-4.0*y/PIXMAP_HEIGHT;         // komplexni konstatny C
            jf=1;
            glutPostRedisplay();
        }
    }
