...
// Ukazkovy priklad cislo 27.2
// Autor: Pavel Tisnovsky
//
// Vykresleni Mandelbrotovy verze fraktalni mnoziny Magnet 2 s moznosti
// nastaveni perturbace.
...
#define WINDOW_TITLE    "Fraktaly 27.2"         // titulek okna
#define WINDOW_WIDTH    512                     // pocatecni velikost okna
#define WINDOW_HEIGHT   384
#define FILE_NAME_1     "fraktaly272a.tga"      // jmeno souboru pro ulozeni pixmapy prvniho fraktalu
#define FILE_NAME_2     "fraktaly272b.tga"      // jmeno souboru pro ulozeni pixmapy druheho fraktalu
...
//-----------------------------------------------------------------------------
// Prekresleni fraktalni mnoziny Magnet 2 s moznosti nastaveni perturbace
//-----------------------------------------------------------------------------
void recalcMagnetM2P(pixmap *pix,               // pixmapa pro vykreslovani
                  ...
                  int    rf, int gf, int bf)    // priznaky barvovych slozek
{
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            zx=pcx;                             // nastavit pocatecni hodnotu Z(0)
            zy=pcy;
...
void onDisplay(void)
{
    ...
    if (ma) {                                   // pokud se ma prepocitat levy fraktal
        recalcMagnetM2P(pixMandelA, maxiter, scale, xpos, ypos, 0.0, 0.0, palette, rf, gf, bf); // prepocet fraktalu
        ma=0;
    }
    ...
    if (mb) {                                   // pokud se ma prepocitat pravy fraktal
        recalcMagnetM2P(pixMandelB, maxiter, scale, xpos, ypos, pcx, pcy, palette, rf, gf, bf); // prepocet fraktalu
        mb=0;
    }
