...
// Ukazkovy priklad cislo 13.3
// Autor: Pavel Tisnovsky
//
// Vykresleni Mandelbrotovy mnoziny s barvovou paletou graficke karty VGA.
// Meze fraktalu jsou nastaveny tak, aby odpovidaly pomeru stran 4:3.
...
#define WINDOW_TITLE    "Fraktaly 13.3"         // titulek okna
#define WINDOW_WIDTH    512                     // pocatecni velikost okna
#define WINDOW_HEIGHT   384
#define FILE_NAME       "fraktaly133.tga"       // jmeno souboru pro ulozeni pixmapy
...
//-----------------------------------------------------------------------------
// Vykresleni pixelu se simulaci sestnactibarevne palety VGA
//-----------------------------------------------------------------------------
void vgaPutpixel(pixmap *pix, int x, int y, int i)
{
    static unsigned char pal[16][3]={           // barvova paleta VGA
        {  0,  0,  0},
        {  0,  0,170},
        {  0,170,  0},
        {  0,170,170},
    ...
    i%=16;                                      // opakovani barev
    putpixel(pix, x, y, pal[i][0], pal[i][1], pal[i][2]);
}
                  ...
                  int    maxiter)               // maximalni pocet iteraci
{
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            ...
            cy=cy0;
            zx=zy=0.0;                          // bude se pocitat orbit nuly
            ...
            vgaPutpixel(pix, x, y, iter);       // vykresleni pixelu
            cx0+=(MAXX-MINX)/pix->width;        // posun na dalsi bod na radku
        }
        cy0+=(MAXY-MINY)/pix->height;           // posun na dalsi radek
    }
}
...
void drawInfo(int maxiter)
{
    ...
    drawString(10, 16, 1.0, 1.0, 1.0, str);
}
...
void onDisplay(void)
{
    ...
    recalcMandelbrot(pix, maxiter);             // prepocet fraktalu
