...
// s vyuzitim deterministickeho algoritmu DIA (Deterministic Iteration Algorithm).
...
#define WINDOW_TITLE    "Fraktaly 33.2"         // titulek okna
#define WINDOW_WIDTH    640                     // pocatecni velikost okna
#define WINDOW_HEIGHT   460
#define FILE_NAME       "fraktaly332.tga"       // jmeno souboru pro ulozeni pixmapy
...
int    depth=40;                                // pocet rekurzivniho zanoreni
...
//-----------------------------------------------------------------------------
// Prekresleni systemu iterovanych funkci IFS pomoci rekurzivniho algoritmu DIA
//-----------------------------------------------------------------------------
void recalcIFSusingDIA(float x, float y, int depth)
{
    float  xn, yn;                                  // poloha iterovaneho bodu
    int    k;                                       // cislo transformace

    if (depth) {
        for (k=0; k<transf; k++) {                  // pro vsechny transformace v IFS
            xn=(*t)[k][0]*x+(*t)[k][1]*y+(*t)[k][4];// provedeni
            ...
            recalcIFSusingDIA(xn, yn, depth-1);     // rekurze - vypocet dalsich "transf" pixelu
        }
    }
}
...
//-----------------------------------------------------------------------------
void drawInfo(double xpos, double ypos, double scale, int ifs, int r, int g, int b, int deltar, int deltag, int deltab, DrawType drawType)
{
#define drawString2(x, y, r, g, b, str) drawString((x)+1, (y), 0.0, 0.0, 0.0, (str)); drawString((x), (y)+1, (r), (g), (b), (str));
    ...
    sprintf(str, "[<][>][,][.] depth: %d/%d=%d", depth, transf, depth/transf);
...
void onDisplay(void)
{
    ...
    if (redraw) {
        red=  rf ? deltar : 0;                  // realne prirustky barvovych slozek
        green=gf ? deltag : 0;
        blue= bf ? deltab : 0;
        clearPixmap(pixIFS);
        xpos+=pixIFS->width/2;
        ypos+=pixIFS->height/2;
        switch (ifs) {                          // vyber IFS systemu
            case 0: t=&IFS_triangle;  transf=3;   break;
            case 1: t=&IFS_tree;      transf=4;   break;
            case 2: t=&IFS_spiral;    transf=2;   break;
            case 3: t=&IFS_spiral2;   transf=5;   break;
            case 4: t=&IFS_dragon;    transf=2;   break;
            case 5: t=&IFS_swirl;     transf=2;   break;
            case 6: t=&IFS_crystal;   transf=2;   break;
            case 7: t=&IFS_Z;         transf=2;   break;
            case 8: t=&IFS_ferny;     transf=2;   break;
            case 9: t=&IFS_fern;      transf=4;   break;
            default:t=&IFS_spiral;    transf=2;   break;
        }
        iter=0;
        recalcIFSusingDIA(0.0f, 0.0f, depth/transf); // rekurzivni prepocet IFS fraktalu
        xpos-=pixIFS->width/2;
        ypos-=pixIFS->height/2;
        redraw=0;
    }
    ...
    glRasterPos2i(0, 0);
    drawInfo(xpos, ypos, scale, ifs, rf, gf, bf, deltar, deltag, deltab, drawType);
...
void onKeyboard(unsigned char key, int x, int y)
{
    ...
    switch (key) {
        ...
        case '<':
        case ',':
                  if (depth>2) depth--; redraw=1; glutPostRedisplay();               break;
        case '>':
        case '.': if (depth<100) depth++; redraw=1; glutPostRedisplay();             break;
