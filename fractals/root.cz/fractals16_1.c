...
// Ukazkovy priklad cislo 16.1
// Autor: Pavel Tisnovsky
//
// Vytvareni animace Mandelbrotovy mnoziny s moznosti volby barvove palety.
...
#define FILE_NAME       "161_"                  // zacatek jmena souboru pro ulozeni pixmapy
...
int main(int argc, char **argv)
{
    ...
    if (argc!=7) {
        puts("wrong # of arguments, use: maxiter palette x1 y1 factor frames");
        return -1;
    }

    // nacteni argumentu
    maxiter=atoi(argv[1]);                      // maximalni pocet iteraci
    palette=atoi(argv[2]);                      // barvova paleta
    x1=atof(argv[3]);                           // souradnice stredu obrazu
    y1=atof(argv[4]);
    factor=atof(argv[5]);                       // zvetseni obrazu
    frames=atoi(argv[6]);                       // celkovy pocet animaci
    ...
    // prvnich dvacet snimku s posunem na zadany stred obrazu (oblasti)
    for (i=0; i<20; i++) {
    ...
    // statickych pet snimku - kratke pozastaveni animace
    for (i=20; i<25; i++) {
        sprintf(name, "%s%03d%s", FILE_NAME, i, ".tga");
        printf("%3d\t", i);
        savePixmap(pix, name);
