...
// Ukazkovy priklad cislo 17.1
// Autor: Pavel Tisnovsky
//
// Vykresleni Mandelbrotovy mnoziny s moznosti nastaveni "perturbace"
...
#define WINDOW_TITLE    "Fraktaly 17.1"         // titulek okna
#define WINDOW_WIDTH    512                     // pocatecni velikost okna
#define WINDOW_HEIGHT   384
#define FILE_NAME_1     "fraktaly171a.tga"      // jmeno souboru pro ulozeni pixmapy nezkreslene Mandelbrotovy mnoziny
#define FILE_NAME_2     "fraktaly171b.tga"      // jmeno souboru pro ulozeni pixmapy zkreslene Mandelbrotovy mnoziny
                  ...
                  double pcy)
{
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            cx=cx0;                             // nastavit pocatecni hodnotu Z(0)
            cy=cy0;
            zx=pcx; zy=pcy;                     // nastaveni perturbace
            for (iter=0; iter<maxiter; iter++) {// iteracni smycka
                zx2=zx*zx;                      // zkraceny vypocet druhe mocniny slozek Z
                zy2=zy*zy;
                if (zx2+zy2>4.0) break;         // kontrola prekroceni meze divergence
