...
// Ukazkovy priklad cislo 20.4
// Autor: Pavel Tisnovsky
//
// Vykresleni Mandelbrotovy mnozinya se Z^4 a moznosti zmeny hodnoty perturbace.
// Pohybem mysi po leve Mandelbrotove mnozine se meni hodnota [pcx, pcy] pro
// pravou Mandelbrotovu mnozinu.
...
#define WINDOW_TITLE    "Fraktaly 20.4"         // titulek okna
#define WINDOW_WIDTH    640                     // pocatecni velikost okna
#define WINDOW_HEIGHT   440
#define FILE_NAME       "fraktaly204.tga"       // jmeno souboru pro ulozeni pixmapy
...
//-----------------------------------------------------------------------------
// Prekresleni Mandelbrotovy mnoziny se Z(n+1)=Z(n)^4+C
//-----------------------------------------------------------------------------
...
{
    double zx, zy, zx2, zy2, zx4, zy4;          // slozky komplexni promenne Z, Z^2 a Z^4
    double zxn, zyn;
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            ...
            for (iter=0; iter<maxiter; iter++) {// iteracni smycka
                ...
                zxn=zx2-zy2;
                zyn=2.0*zx*zy;                  // nyni mame vypocteno Z^2
                zx4=zxn*zxn;
                zy4=zyn*zyn;
                zy=2.0*zxn*zyn+cy;              // ted je vypocteno Z^4
                zx=zx4-zy4+cx;
            }
...
//-----------------------------------------------------------------------------
// Prekresleni Mandelbrotovy mnoziny se Z(n+1)=Z(n)^4+C s prednastavenou
// hodnotou perturbace
//-----------------------------------------------------------------------------
void recalcMandelbrotPert( pixmap *pix,         // pixmapa pro vykreslovani
                  int    maxiter,               // maximalni pocet iteraci
                  double scale,                 // meritko obrazce
                  double xpos,                  // posun obrazce
                  double ypos,
...
void drawInfo(int maxiter, double xpos, double ypos, double scale, int palette, int r, int g, int b, double cx, double cy)
{
    ...
    sprintf(str, "[Mouse]   perturbation = %5.3f, %5.3f", cx, cy);
...
void onDisplay(void)
{
    ...
    if (ma) {                                   // pokud se ma prepocitat Mandelbrotova mnozina
        recalcMandelbrot(pixMandel, maxiter, scale, xpos, ypos, palette, rf, gf, bf);// prepocet fraktalu
        ma=0;
    }
    if (mb) {                                   // pokud se ma prepocitat Juliova mnozina
        recalcMandelbrotPert(pixJulia, maxiter, scale, xpos, ypos, palette, rf, gf, bf, pcx, pcy);// prepocet fraktalu
        mb=0;
    }
