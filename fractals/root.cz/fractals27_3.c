...
// Ukazkovy priklad cislo 27.3
// Autor: Pavel Tisnovsky
//
// Vykresleni fraktalni mnoziny Magnet 2 spolu s jeji Juliovou verzi.
...
#define WINDOW_TITLE    "Fraktaly 27.3"         // titulek okna
#define WINDOW_WIDTH    512                     // pocatecni velikost okna
#define WINDOW_HEIGHT   384
#define FILE_NAME_1     "fraktaly273m.tga"      // jmeno souboru pro ulozeni pixmapy prvniho fraktalu
#define FILE_NAME_2     "fraktaly273j.tga"      // jmeno souboru pro ulozeni pixmapy druheho fraktalu
...
int    palette=4;                               // barvova paleta
int    maxiter=255;                             // maximalni pocet iteraci
int    rf=1, gf=1, bf=1;                        // priznaky barvovych slozek
double scale=1.0;                               // meritko fraktalu
double xpos=1.0;                                // posun fraktalu
...
//-----------------------------------------------------------------------------
// Prekresleni fraktalni mnoziny Magnet 2 v Mandelbrotove variante
//-----------------------------------------------------------------------------
void recalcMagnetM2(pixmap *pix,                // pixmapa pro vykreslovani
                  ...
                  int    rf, int gf, int bf)    // priznaky barvovych slozek
{
    ...
    double xmin, ymin, xmax, ymax;              // rohy vykreslovaneho obrazce v komplexni
                                                // rovine
    double div;                                 // hodnota, kterou se bude vysledek delit
                                                // pred umocnenim
    double tzx, tzy, bzx, bzy;                  // citatel a jmenovatel zlomku
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            zx=0.0;                             // nastavit pocatecni hodnotu Z(0)
            zy=0.0;
            cm1x=cx0-1.0; cm1y=cy0;             // predpocitane parametry
            ...
            for (iter=0; iter<maxiter; iter++) {// iteracni smycka
                ...
                zy2=zy*zy;
                tzx=zx*(zx2-3.0*zy2+cm1x)-zy*cm1y+cm1cm2x; // citatel zlomku
...
#define MIN_VALUE 1.0-100
                if (div<MIN_VALUE) break;       // zajistit, aby nedoslo k deleni nulou
                zxn=(tzx*bzx+tzy*bzy)/div;
                zyn=(tzy*bzx-tzx*bzy)/div;
                zx=(zxn+zyn)*(zxn-zyn);         // z=zn^2
                zy=2.0*zxn*zyn;
                if (zx2+zy2>100*100) break;
                if (((zx-1.0)*(zx-1.0)+zy*zy)<0.001) break; // test na fixni bod
            }
...
//-----------------------------------------------------------------------------
// Prekresleni frakalni mnoziny Magnet 2 v Juliove variante
//-----------------------------------------------------------------------------
void recalcMagnetJ2(pixmap *pix,                // pixmapa pro vykreslovani
                  int    maxiter,               // maximalni pocet iteraci
                  double scale,                 // meritko obrazce
                  double xpos,                  // posun obrazce
                  double ypos,
                  double cx,
                  double cy,                    // poloha bodu v komplexni rovine
                  int    palette,               // barvova paleta
                  int    rf, int gf, int bf)    // priznaky barvovych slozek
{
    double zx, zy, zx2, zy2, zxn, zyn;          // slozky komplexni promenne Z, Z^2 a Z^3
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        cx0=xmin;
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            zx=cx0;                             // nastavit pocatecni hodnotu Z(0)
            zy=cy0;
            cm1x=cx-1.0; cm1y=cy;               // predpocitane parametry
            cm2x=cx-2.0; cm2y=cy;
            cm1cm2x=cm1x*cm2x-cm1y*cm2y;
            cm1cm2y=cm1x*cm2y+cm1y*cm2x;
            cm1x+=cm1x+cm1x; cm1y+=cm1y+cm1y;
...
//-----------------------------------------------------------------------------
void drawInfo(int maxiter, double pcx, double pcy, double xpos, double ypos, double scale, int palette, int r, int g, int b)
{
    char str[100];
    char *palNames[10]={
        "blues",
    ...
    drawString(11,111, 0.0, 0.0, 0.0, str);     // stinovani
    drawString(10,112, 0.6, 1.0, 0.6, str);
    sprintf(str, "[0]-[9]    palette = %s", palNames[palette]);
    drawString(11, 95, 0.0, 0.0, 0.0, str);
...
void onDisplay(void)
{
    ...
    if (ma) {                                   // pokud se ma prepocitat levy fraktal
        recalcMagnetM2(pixMandelA, maxiter, scale, xpos, ypos, palette, rf, gf, bf); // prepocet fraktalu
        ma=0;
    }
    ...
    if (mb) {                                   // pokud se ma prepocitat pravy fraktal
        recalcMagnetJ2(pixMandelB, maxiter, scale, xpos, ypos, pcx, pcy, palette, rf, gf, bf); // prepocet fraktalu
        mb=0;
    }
    ...
    glRasterPos2i(0, 0);
    drawInfo(maxiter, pcx, pcy, xpos,ypos, scale, palette, rf, gf, bf);                  // vypsani informaci o fraktalu
...
void onKeyboard(unsigned char key, int x, int y)
{
    ...
    switch (key) {
        ...
        case 's': savePixmap(pixMandelA, FILE_NAME_1);
                  savePixmap(pixMandelB, FILE_NAME_2); break; // ulozeni pixmapy
        case '<': if (maxiter>10)   maxiter-=10; ma=1; glutPostRedisplay();  break; 
        case ',': if (maxiter>1)    maxiter--;   ma=1; glutPostRedisplay();  break; // zmena poctu iteraci
        case '>': if (maxiter<246)  maxiter+=10; ma=1; glutPostRedisplay();  break; 
        case '.': if (maxiter<255)  maxiter++;   ma=1; glutPostRedisplay();  break; 
...
void onSpecial(int key, int x, int y)
{
    ...
    switch (key) {
        case GLUT_KEY_LEFT:      xpos-=0.05/scale; ma=mb=1; glutPostRedisplay();        break;
        case GLUT_KEY_RIGHT:     xpos+=0.05/scale; ma=mb=1; glutPostRedisplay();        break;
        case GLUT_KEY_UP:        ypos+=0.05/scale; ma=mb=1; glutPostRedisplay();        break;
        case GLUT_KEY_DOWN:      ypos-=0.05/scale; ma=mb=1; glutPostRedisplay();        break;
        case GLUT_KEY_PAGE_UP:   scale*=1.1; ma=mb=1; glutPostRedisplay();              break;
        case GLUT_KEY_PAGE_DOWN: if (scale>1) scale/=1.1; ma=mb=1; glutPostRedisplay(); break;
        default:                                                                        break;
    }
}
#ifdef __BORLANDC__
