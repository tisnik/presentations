...
// Ukazkovy priklad cislo 19.6
// Autor: Pavel Tisnovsky
//
// Vykresleni kubicke Mandelbrotovy mnoziny s moznosti zmeny pohledu na mnozinu
// a zmeny pouzite barvove palety. Body lezici vne mnoziny jsou obarveny na
// zaklade uhlu orbitu.
...
#define WINDOW_TITLE    "Fraktaly 19.6"         // titulek okna
#define WINDOW_WIDTH    640                     // pocatecni velikost okna
#define WINDOW_HEIGHT   440
#define FILE_NAME       "fraktaly196.tga"       // jmeno souboru pro ulozeni pixmapy
...
void recalcCubicMandelbrot( pixmap *pix,        // pixmapa pro vykreslovani
                  ...
                  double pertx,
                  double perty)                 // aktualne nastavena hodnota perturbace
{
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            ...
            cy=cy0;
            zx=pertx;                           // nastaveni hodnoty perturbace
            zy=perty;
...
void onDisplay(void)
{
    ...
    if (ma) {                                   // pokud se ma prepocitat Mandelbrotova mnozina
        recalcCubicMandelbrot(pixMandelA, maxiter, scale, xpos, ypos, palette, rf, gf, bf, 0.0, 0.0);// prepocet fraktalu
        ma=0;
    }
    if (mb) {                                   // pokud se ma prepocitat zkreslena Mandelbrotova mnozina
        recalcCubicMandelbrot(pixMandelB, maxiter, scale, xpos, ypos, palette, rf, gf, bf, pcx, pcy);// prepocet fraktalu
        mb=0;
    }
...
void onKeyboard(unsigned char key, int x, int y)
{
    ...
    switch (key) {
        ...
        case 's': savePixmap(pixMandelB, FILE_NAME);                          break; // ulozeni pixmapy
