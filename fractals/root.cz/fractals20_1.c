...
// Ukazkovy priklad cislo 20.1
// Autor: Pavel Tisnovsky
//
// Vykresleni kubicke Mandelbrotovy mnoziny a Juliovy mnoziny. Pohybem mysi
// po Mandelbrotove mnozine se meni hodnota [cx,cy] pro Juliovu mnozinu.
...
#define WINDOW_TITLE    "Fraktaly 20.1"         // titulek okna
#define WINDOW_WIDTH    640                     // pocatecni velikost okna
#define WINDOW_HEIGHT   440
#define FILE_NAME       "fraktaly201.tga"       // jmeno souboru pro ulozeni pixmapy

#define PIXMAP_WIDTH    320                     // sirka pixmapy
#define PIXMAP_HEIGHT   320                     // vyska pixmapy
...
pixmap *pixJulia;                               // pixmapa pro obrazek Juliovy
...
double pcx=0.0;                                 // hodnota perturbace
double pcy=0.0;
...
//-----------------------------------------------------------------------------
// Prekresleni kubicke Mandelbrotovy mnoziny
//-----------------------------------------------------------------------------
void recalcCubicMandelbrot( pixmap *pix,        // pixmapa pro vykreslovani
...
{
    double zx, zy, zx2, zy2, zx3, zy3;          // slozky komplexni promenne Z, Z^2 a Z^3
    double zxn, zyn;
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            ...
            for (iter=0; iter<maxiter; iter++) {// iteracni smycka
                ...
                zy2=zy*zy;
                zx3=zx2*zx;                     // zkraceny vypocet treti mocniny slozek Z
                zy3=zy2*zy;
...
//-----------------------------------------------------------------------------
// Prekresleni kubicke Juliovy mnoziny
//-----------------------------------------------------------------------------
void recalcCubicJulia( pixmap *pix,             // pixmapa pro vykreslovani
                  int    maxiter,               // maximalni pocet iteraci
                  double scale,                 // meritko obrazce
                  double xpos,                  // posun obrazce
                  double ypos,
                  ...
                  int    rf, int gf, int bf,    // priznaky barvovych slozek
                  double cx0,
                  double cy0)                   // aktualne nastavena hodnota C
{
    double zx, zy, zx2, zy2, zx3, zy3;          // slozky komplexni promenne Z, Z^2 a Z^3
    double zxn, zyn;
    double cx, cy;                              // slozky komplexni konstanty C
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            ...
            if (iter==maxiter)                  // pixely uvnitr Juliovy
                r=g=b=0;                        // mnoziny jsou cerne
            else                                // vypocet barev podle poctu iteraci
                mapPalette(palette, iter, &r, &g, &b);
            r=r*rf;                             // uzivatelem rizene vynulovani
...
//-----------------------------------------------------------------------------
void drawInfo(int maxiter, double xpos, double ypos, double scale, int palette, int r, int g, int b, double cx, double cy)
{
    char str[100];
    char *palNames[10]={
        "blues",
    ...
    drawString(11, 94, 0.0, 0.0, 0.0, str);     // stinovani
    drawString(10, 95, 0.6, 1.0, 0.6, str);
    sprintf(str, "[0]-[9]    palette = %s", palNames[palette]);
    drawString(11, 78, 0.0, 0.0, 0.0, str);
    drawString(10, 79, 0.6, 1.0, 0.6, str);
    sprintf(str, "[<][>]     maxiter = %d", maxiter);
    drawString(11, 62, 0.0, 0.0, 0.0, str);
    drawString(10, 63, 0.6, 1.0, 0.6, str);
    sprintf(str, "[PgUp][PgDn]  scale = %4.2f ", scale);
    drawString(11, 46, 0.0, 0.0, 0.0, str);
    drawString(10, 47, 0.4, 1.0, 0.8, str);
    sprintf(str, "[Arrows]  pan = %5.3f, %5.3f", xpos, ypos);
    drawString(11, 30, 0.0, 0.0, 0.0, str);
    drawString(10, 31, 0.2, 1.0, 1.0, str);
    sprintf(str, "[Mouse]   c = %5.3f, %5.3f", cx, cy);
    drawString(11, 15, 0.0, 0.0, 0.0, str);
    drawString(10, 16, 0.8, 0.2, 0.2, str);
}
...
void onDisplay(void)
{
    ...
    if (ma) {                                   // pokud se ma prepocitat Mandelbrotova mnozina
        recalcCubicMandelbrot(pixMandel, maxiter, scale, xpos, ypos, palette, rf, gf, bf);// prepocet fraktalu
        ma=0;
    }
    if (mb) {                                   // pokud se ma prepocitat Juliova mnozina
        recalcCubicJulia(pixJulia, maxiter, scale, xpos, ypos, palette, rf, gf, bf, pcx, pcy);// prepocet fraktalu
        mb=0;
    }
    ...
    drawPixmap(pixJulia);
    glRasterPos2i(0, 0);
    drawInfo(maxiter, xpos, ypos, scale, palette, rf, gf, bf, pcx, pcy);
...
void onKeyboard(unsigned char key, int x, int y)
{
    ...
    if (key>='0' && key<='9') { palette=key-'0'; ma=mb=1; glutPostRedisplay(); } // nastaveni barvove palety
    switch (key) {
        case 27:                                                                     // pokud byla stlacena klavesa ESC, konec programu
        case 'q': exit(0);                                                    break; // totez co klavesa ESC
        case 's': savePixmap(pixJulia, FILE_NAME);                            break; // ulozeni pixmapy
        case '<': if (maxiter>10)  maxiter-=10; ma=mb=1; glutPostRedisplay(); break; 
        case ',': if (maxiter>1)   maxiter--;   ma=mb=1; glutPostRedisplay(); break; // zmena poctu iteraci
        case '>': if (maxiter<246) maxiter+=10; ma=mb=1; glutPostRedisplay(); break; 
        case '.': if (maxiter<255) maxiter++;   ma=mb=1; glutPostRedisplay(); break; 
        case 'r': rf=!rf;                       ma=mb=1; glutPostRedisplay(); break; // priznaky barvovych slozek
        case 'g': gf=!gf;                       ma=mb=1; glutPostRedisplay(); break;
        case 'b': bf=!bf;                       ma=mb=1; glutPostRedisplay(); break;
        default:                                                              break; 
    }
}
#ifdef __BORLANDC__
...
void onSpecial(int key, int x, int y)
{
    ...
    switch (key) {
        case GLUT_KEY_LEFT:      ma=mb=1; xpos-=0.05/scale; glutPostRedisplay();           break;
        case GLUT_KEY_RIGHT:     ma=mb=1; xpos+=0.05/scale; glutPostRedisplay();           break;
        case GLUT_KEY_UP:        ma=mb=1; ypos+=0.05/scale; glutPostRedisplay();           break;
        case GLUT_KEY_DOWN:      ma=mb=1; ypos-=0.05/scale; glutPostRedisplay();           break;
        case GLUT_KEY_PAGE_UP:   ma=mb=1; scale*=1.1; glutPostRedisplay();                 break;
        case GLUT_KEY_PAGE_DOWN: ma=mb=1; if (scale>1) scale/=1.1; glutPostRedisplay();    break;
        default:                                                                           break;
    }
}
#ifdef __BORLANDC__
