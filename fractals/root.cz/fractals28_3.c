...
// Ukazkovy priklad cislo 28.3
// Autor: Pavel Tisnovsky
//
// Vykresleni Mandelbrotovy verze fraktalni mnoziny Phoenix s moznosti
// nastaveni nenulove hodnoty perturbace.
...
#define WINDOW_TITLE    "Fraktaly 28.3"         // titulek okna
#define WINDOW_WIDTH    512                     // pocatecni velikost okna
#define WINDOW_HEIGHT   384
#define FILE_NAME_1     "fraktaly283a.tga"      // jmeno souboru pro ulozeni pixmapy prvniho fraktalu
#define FILE_NAME_2     "fraktaly283b.tga"      // jmeno souboru pro ulozeni pixmapy druheho fraktalu
...
//-----------------------------------------------------------------------------
// Prekresleni Mandelbrotovy verze fraktalni mnoziny typu Phoenix s moznosti
// nastaveni nenulove hodnoty perturbace
//-----------------------------------------------------------------------------
void recalcPhoenixMP(pixmap *pix,               // pixmapa pro vykreslovani
                  int    maxiter,               // maximalni pocet iteraci
                  double scale,                 // meritko obrazce
                  double xpos,                  // posun obrazce
                  double ypos,
                  double pcx,                   // nastavena hodnota perturbace
                  double pcy,
                  ...
                  int    rf, int gf, int bf)    // priznaky barvovych slozek
{
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            zx=cx0+pcx;                         // nastavit pocatecni hodnotu Z(0)
            zy=cy0+pcy;
...
void onDisplay(void)
{
    ...
    if (ma) {                                   // pokud se ma prepocitat levy fraktal
        recalcPhoenixMP(pixMandelA, maxiter, scale, xpos, ypos, 0.0, 0.0, palette, rf, gf, bf); // prepocet fraktalu
        ma=0;
    }
    ...
    if (mb) {                                   // pokud se ma prepocitat pravy fraktal
        recalcPhoenixMP(pixMandelB, maxiter, scale, xpos, ypos, pcx, pcy, palette, rf, gf, bf); // prepocet fraktalu
        mb=0;
    }
