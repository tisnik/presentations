...
// pomoci klavesove zkratky [S]. Klavesami [0]-[9] se meni pouzita barvova
// paleta pri vykreslovani plasmy.
...
#define WINDOW_TITLE    "Fraktaly 47.2"         // titulek okna
...
#define FILE_NAME       "fraktaly47_2.tga"      // jmeno souboru s ulozenym obrazkem
...
void recalcFractal(void)
{
    ...
    for (y=0; y<PIXMAP_HEIGHT; y++)
        for (x=0; x<PIXMAP_WIDTH; x++) {
            ...
            if (fmin>f) fmin=f;
            if (fmax<f) fmax=f;
            mapPalette(palette, (int)f, &r, &g, &b); // mapovani barvove palety
            putpixel(pix, x, y, r, g, b);    // ulozeni pixelu
        }
    printf("fmin=%f\nfmax=%f\n", fmin, fmax);
}
...
{
    if (key>='0' && key<='9') {
        palette=key-'0';                       // nastaveni barvove palety
        recalcFractal();
        glutPostRedisplay();
        return;
    }
