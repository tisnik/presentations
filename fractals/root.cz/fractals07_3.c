...
// Ukazkovy priklad cislo 7.3
// Autor: Pavel Tisnovsky
//
// Vykresleni dynamickeho systemu nazvaneho Latoocarfian.
...
#define WINDOW_TITLE    "Fraktaly 7.3"          // titulek okna
#define WINDOW_WIDTH    512                     // pocatecni velikost okna
#define WINDOW_HEIGHT   384
#define FILE_NAME       "fraktaly73.tga"        // jmeno souboru pro ulozeni pixmapy
...
double a=-0.966;                                // parametry fraktalu
double b=2.879;
double c=0.765;
double d=0.744;
int    maxiter=900000;                          // maximalni pocet iteraci
double scale=100;                               // meritko fraktalu
              ...
              const unsigned int y)
{
    ...
    if (!p || !p->pixels || x>=p->width || y>=p->height) return;
    pos=3*(x+y*p->width);
    p->pixels[pos++]+=2;                        // nastaveni barvy pixelu
    p->pixels[pos++]+=2;
    p->pixels[pos]+=2;
}
...
//-----------------------------------------------------------------------------
void recalcLatoocarfian(pixmap *pix,            // pixmapa pro vykreslovani
                    double a,                   // parametry fraktalu
                    double b,
                    double c,
                    double d,
                    ...
                    double pany)
{
    ...
    while (iter--) {
        ...
        x=x2;
        y=y2;
        //putpixel(pix, x*scale+panx, y*scale+pany, iter/10, iter/50, iter/80);// vykresleni pixelu
        addluminance(pix, x*scale+panx, y*scale+pany);// vykresleni pixelu
    }
}
...
//-----------------------------------------------------------------------------
void drawInfo(double a, double b, double c, double d, int maxiter, double scale, double panx, double pany)
{
    char str[100];
...
void onDisplay(void)
{
    ...
    recalcLatoocarfian(pix, a, b, c, d, maxiter, scale, panx, pany); // prepocet fraktalu
    drawPixmap(pix);                            // vykresleni pixmapy
    drawInfo(a, b, c, d, maxiter, scale, panx, pany);
...
void onKeyboard(unsigned char key, int x, int y)
{
    ...
    switch (key) {
        ...
        case '5': c-=0.1;                          glutPostRedisplay(); break;
        case '6': c+=0.1;                          glutPostRedisplay(); break;
        case '7': d-=0.1;                          glutPostRedisplay(); break;
        case '8': d+=0.1;                          glutPostRedisplay(); break;
        case '<':
        case ',': if (maxiter>10000) maxiter-=10000;   glutPostRedisplay(); break; // zmena poctu iteraci
        case '>':
        case '.': if (maxiter<1000000) maxiter+=10000; glutPostRedisplay(); break;
        default:                                                        break;
    }
}
#ifdef __BORLANDC__
