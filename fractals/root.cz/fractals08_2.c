...
// Ukazkovy priklad cislo 8.2
// Autor: Pavel Tisnovsky
//
// Vykresleni dynamickeho systemu Popcorn s orbity zobrazenymi pro ruzne
// pocatecni podminky.
...
#define WINDOW_TITLE    "Fraktaly 8.2"          // titulek okna
#define WINDOW_WIDTH    512                     // pocatecni velikost okna
#define WINDOW_HEIGHT   384
#define FILE_NAME       "fraktaly82.tga"        // jmeno souboru pro ulozeni pixmapy
...
double scale=60;                                // meritko fraktalu
double panx=256;                                // posun fraktalu
double pany=192;                                // posun fraktalu

// zmena realne a imaginarni multiplikativni konstanty
#define STEP_R 0.00
#define STEP_I 0.00
#define CR     3.0
#define CI     0.0
                  ...
                  double pany)
{
    ...
    double cr, ci=CI;
    int iter;
    for (y0=MINY; y0<MAXY; y0+=STEP_Y) {     // "mrizka" pocatecnich podminek
        cr=CR;
        for (x0=MINX; x0<MAXX; x0+=STEP_X) {
            ...
            cr+=STEP_R;                      // zmena realne casti multiplikativni konstanty
        }
        ci+=STEP_I;                          // zmena imaginarni casti multiplikativni konstanty
    }
}
...
void onDisplay(void)
{
    ...
    recalcDynamic(pix, maxiter, scale, panx, pany); // prepocet fraktalu
