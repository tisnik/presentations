...
// Ukazkovy priklad cislo 25.1
// Autor: Pavel Tisnovsky
//
// Vykresleni Juliovy verze Barnsleyovy fraktalni mnoziny J1.
...
#define WINDOW_TITLE    "Fraktaly 25.1"         // titulek okna
#define WINDOW_WIDTH    640                     // pocatecni velikost okna
#define WINDOW_HEIGHT   460
#define FILE_NAME       "fraktaly251.tga"       // jmeno souboru pro ulozeni pixmapy
...
//-----------------------------------------------------------------------------
// Prekresleni Mandelbrotovy verze Barnsleyovy fraktalni mnoziny M1
//-----------------------------------------------------------------------------
void recalcMandelbrotM1(pixmap *pix,            // pixmapa pro vykreslovani
                  ...
                  int    rf, int gf, int bf)    // priznaky barvovych slozek
{
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            zx=0;                               // nastavit pocatecni hodnotu Z(0)
            zy=0;
...
//-----------------------------------------------------------------------------
// Prekresleni Juliovy verze Barnsleyovy fraktalni mnoziny J1
//-----------------------------------------------------------------------------
void recalcJuliaJ1(pixmap *pix,                 // pixmapa pro vykreslovani
                  int    maxiter,               // maximalni pocet iteraci
                  double scale,                 // meritko obrazce
                  double xpos,                  // posun obrazce
                  double ypos,
...
void onDisplay(void)
{
    ...
    if (ma) {                                   // pokud se ma prepocitat Mandelbrotova mnozina
        recalcMandelbrotM1(pixMandel, maxiter, scale, xpos, ypos, palette, rf, gf, bf);// prepocet fraktalu
        ma=0;
    }
    if (mb) {                                   // pokud se ma prepocitat Juliova mnozina
        recalcJuliaJ1(pixJulia, maxiter, scale, xpos, ypos, palette, rf, gf, bf, pcx, pcy);// prepocet fraktalu
        mb=0;
    }
