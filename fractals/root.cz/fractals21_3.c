...
// Ukazkovy priklad cislo 21.3
// Autor: Pavel Tisnovsky
//
// Vytvareni animace Mandelbrotovy mnoziny o libovolne mocnine s moznosti volby
// pouzite barvove palety.
// Tento program vytvori serii 24bitovych obrazku, ktere budou obsahovat snimky
// animace prubezne zmeny mocniny Mandelbrotovy mnoziny.
...
#define FILE_NAME       "213_"                  // zacatek jmena souboru pro ulozeni pixmapy

#define PIXMAP_WIDTH    (352*2)                 // sirka pixmapy
#define PIXMAP_HEIGHT   (288*2)                 // vyska pixmapy
...
int main(int argc, char **argv)
{
    ...
    power=power_s;
    // prubezna zmena mocniny
    for (i=0; i<frames; i++) {
        sprintf(name, "%s%03d%s", FILE_NAME, i, ".tga");
        printf("%3d\t", i);
        recalcMandelbrot(pix, maxiter, 1.0, 0, 0, power, palette);
        savePixmap(pix, name);
        power+=(power_e-power_s)/frames;
    }
    printf("\ndone!\n");
