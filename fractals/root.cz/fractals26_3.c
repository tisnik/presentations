...
// Ukazkovy priklad cislo 26.3
// Autor: Pavel Tisnovsky
//
// Vykresleni fraktalni mnoziny Magnet 1 spolu s jeji Juliovou verzi.
...
#define WINDOW_TITLE    "Fraktaly 26.3"         // titulek okna
#define WINDOW_WIDTH    512                     // pocatecni velikost okna
#define WINDOW_HEIGHT   384
#define FILE_NAME_1     "fraktaly263m.tga"      // jmeno souboru pro ulozeni pixmapy prvniho fraktalu
#define FILE_NAME_2     "fraktaly263j.tga"      // jmeno souboru pro ulozeni pixmapy druheho fraktalu
...
//-----------------------------------------------------------------------------
// Prekresleni fraktalni mnoziny Magnet 1
//-----------------------------------------------------------------------------
void recalcMagnetM1(pixmap *pix,                // pixmapa pro vykreslovani
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            ...
            for (iter=0; iter<maxiter; iter++) {// iteracni smycka
                ...
                tzx=zx2-zy2+cx0-1;              // top=z^2+c-1
                tzy=2.0*zx*zy+cy0;
                bzx=2.0*zx+cx0-2;               // bot=2z+c-2
                bzy=2.0*zy+cy0;
...
//-----------------------------------------------------------------------------
// Prekresleni frakalni mnoziny Magnet 1 v Juliove variante
//-----------------------------------------------------------------------------
void recalcMagnetJ1(pixmap *pix,                // pixmapa pro vykreslovani
                  int    maxiter,               // maximalni pocet iteraci
                  double scale,                 // meritko obrazce
                  double xpos,                  // posun obrazce
                  double ypos,
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            ...
            for (iter=0; iter<maxiter; iter++) {// iteracni smycka
                ...
                if (zx2+zy2>100) break;
                if (((zx-1.0)*(zx-1.0)+zy*zy)<0.001) break;
                tzx=zx2-zy2+cx-1;              // top=z^2+c-1
                tzy=2.0*zx*zy+cy;
                bzx=2.0*zx+cx-2;               // bot=2z+c-2
                bzy=2.0*zy+cy;
                div=bzx*bzx+bzy*bzy;            // zn=top/bot
#define MIN_VALUE 1.0-100
                if (div<MIN_VALUE) break;       // zajistit, aby nedoslo k deleni nulou
...
void onDisplay(void)
{
    ...
    if (ma) {                                   // pokud se ma prepocitat levy fraktal
        recalcMagnetM1(pixMandelA, maxiter, scale, xpos, ypos, palette, rf, gf, bf); // prepocet fraktalu
        ma=0;
    }
    ...
    if (mb) {                                   // pokud se ma prepocitat pravy fraktal
        recalcMagnetJ1(pixMandelB, maxiter, scale, xpos, ypos, pcx, pcy, palette, rf, gf, bf); // prepocet fraktalu
        mb=0;
    }
