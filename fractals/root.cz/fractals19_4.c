...
// zaklade souctu realne a imaginarni slozky orbitu a soucasne i poctu iteraci.
...
#define WINDOW_TITLE    "Fraktaly 19.4"         // titulek okna
#define WINDOW_WIDTH    512                     // pocatecni velikost okna
#define WINDOW_HEIGHT   384
#define FILE_NAME       "fraktaly194.tga"       // jmeno souboru pro ulozeni pixmapy
                  ...
                  int    rf, int gf, int bf)    // priznaky barvovych slozek
{
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            cx=cy0;                             // nastavit pocatecni hodnotu Z(0)
            cy=cx0;
            ...
            if (iter==maxiter)                  // pixely uvnitr Mandelbrotovy
                r=g=b=0;                        // mnoziny jsou cerne
                                                ...
                                                // a realne i imaginarni slozky orbitu
...
void onDisplay(void)
{
    ...
    recalcCubicMandelbrot(pix, maxiter, scale, xpos, ypos, palette, rf, gf, bf);// prepocet fraktalu
