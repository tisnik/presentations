...
// Ukazkovy priklad cislo 28.2
// Autor: Pavel Tisnovsky
//
// Vykresleni fraktalni mnoziny Phoenix spolu s jeji Juliovou verzi.
...
#define WINDOW_TITLE    "Fraktaly 28.2"         // titulek okna
#define WINDOW_WIDTH    512                     // pocatecni velikost okna
#define WINDOW_HEIGHT   384
#define FILE_NAME_1     "fraktaly282m.tga"      // jmeno souboru pro ulozeni pixmapy prvniho fraktalu
#define FILE_NAME_2     "fraktaly282j.tga"      // jmeno souboru pro ulozeni pixmapy druheho fraktalu
...
//-----------------------------------------------------------------------------
// Prekresleni fraktalni mnoziny Phoenix v Mandelbrotove variante
//-----------------------------------------------------------------------------
void recalcPhoenixM(pixmap *pix,                // pixmapa pro vykreslovani
...
{
    double zx, zy, zx2, zy2, zxn, zyn;          // slozky komplexni promenne Z a Z^2
    double cx0, cy0;
    ...
    for (y=0; y<pix->height; y++) {             // pro vsechny radky v pixmape
        ...
        for (x=0; x<pix->width; x++) {          // pro vsechny pixely na radku
            ...
            zy=cy0;
            ynx=0.0;                            // nastavit pocatecni hodnotu Y(0)
            yny=0.0;
...
//-----------------------------------------------------------------------------
// Prekresleni frakalni mnoziny Phoenix v Juliove variante
//-----------------------------------------------------------------------------
void recalcPhoenixJ(pixmap *pix,                // pixmapa pro vykreslovani
                  int    maxiter,               // maximalni pocet iteraci
                  double scale,                 // meritko obrazce
                  double xpos,                  // posun obrazce
                  double ypos,
...
void onDisplay(void)
{
    ...
    if (ma) {                                   // pokud se ma prepocitat levy fraktal
        recalcPhoenixM(pixMandelA, maxiter, scale, xpos, ypos, palette, rf, gf, bf); // prepocet fraktalu
        ma=0;
    }
    ...
    if (mb) {                                   // pokud se ma prepocitat pravy fraktal
        recalcPhoenixJ(pixMandelB, maxiter, scale, xpos, ypos, pcx, pcy, palette, rf, gf, bf); // prepocet fraktalu
        mb=0;
    }
