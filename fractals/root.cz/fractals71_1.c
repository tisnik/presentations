...
//
// Demonstracni priklad na vypocet moare s kruznicovym vzorkem
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void recalcCirclePattern(pixmap *pix,           // pracovni pixmapa
                    ...
                    double ymax)
{
    ...
    double x1, y1, x2, y2;                      // prepoctena pozice pixelu
...
int main(int argc, char **argv)
{
    ...
    // vykreslit nekolik moare
    for (i=0; i<50; i++) {
        char str[20];
        double mez;
        sprintf(str, "circle%02d.tga", i);
        printf("making image '%s'\n", str);
        mez=(2<<5)+i*2.5;
        recalcCirclePattern(pix, i%10, -mez, -mez, mez, mez);
        savePixmap(pix, str);
    }
    puts("all done");
    return 0;
